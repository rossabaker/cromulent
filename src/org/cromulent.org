...
For a single-user Libravatar, we need just a few config options:

- A =domain= to host an ~/avatar~ path on.

A NixOS nginx configuration follows.  A location is added for each
hash that serves ~avatar.jpg~ from the =root= directory.
