...
:PROPERTIES:
:EXPORT_FILE_NAME: _index
:EXPORT_HUGO_LASTMOD: [2023-11-09 Thu 21:52 EST]
...
:EXPORT_HUGO_LASTMOD: [2023-11-28 Tue 11:51 EST]
:header-args+: :code-license MIT
:EXPORT_OPTIONS: h:6
:END:
...
At the root of the [[https://git.rossabaker.com/ross/cromulent][Cromulent repository]] is a [[https://nixos.wiki/wiki/Flakes][Nix flake]].  It contains
...
#+begin_src sh :eval yes :exports code :results output verbatim :code-license CC0-1.0
  nix flake show git+https://git.rossabaker.com/ross/cromulent
#+end_src
...
:EXPORT_HUGO_LASTMOD: [2023-12-13 Wed 20:25 EST]
:CUSTOM_ID: abe
...
Define the OpenTofu variable to match
~com.rossabaker.services.libravatar~, plus a trailing period.

#+begin_src terraform :tangle ../../gen/infra/hetznerdns/main.auto.tfvars
  cromulent_avatars_domain = "avatars.rossabaker.com."
#+end_src

~avatars.rossabaker.com~ is a {{{abbr(common name,~CNAME)}}} for
~abe~.

#+begin_src terraform :tangle ../../gen/infra/hetznerdns/main.tf
...
~abe~ is the owner of several DNS records, as [[#dns][managed by OpenTofu]].
Its full name is ~abe.hetzner.rossabaker.com~.

#+begin_aside
This haphazardly weaves OpenTofu with Nix, such that the inclusion of
a module requires a DNS entry managed by another tool.  [[https://terranix.org/][Terranix]] might
be a solution.
#+end_aside

***** IP addresses

Foremost is the server address.  ~A~ is IPv4, and ~AAAA~ is IPv6.
...
[[https://github.com/Janik-Haag/NixOS-DNS/][NixOS-DNS]].  This base module enables its networking module.

The actual DNS records are defined in =networking.domains.subDomains=,
but each gets its defaults from the closest matching
=networking.domains.baseDomains=.  We don't specify any defaults, but
if there is no matching record in =baseDomains=, no record is
generated at all!

#+begin_warning
Forgetting =baseDomains= is [[https://janik-haag.github.io/NixOS-DNS/usage.html#nixos-module][documented as an error]], but I see silent
failure.
#+end_warning
...
A ~CNAME~ record is necessary for all the services we host.
...
When we mixed in certain NixOS services, we also took responsibility
for the subdomains they declared.
...
:header-args:nix+: :code-license MIT
:EXPORT_HUGO_LASTMOD: [2023-12-05 Tue 21:42 EST]
:END:
...
** DONE Declarative DNS with Hetzner DNS and OpenTofu
...
:header-args:terraform+: :code-license CC0-1.0
:EXPORT_HUGO_LASTMOD: [2023-12-13 Wed 21:17 EST]
:END:

#+begin_description
Sets up a DNS zone for rossabaker.com in Hetzner DNS with OpenTofu.
#+end_description

The services hosted at ~rossabaker.com~ require multiple DNS entries.
[[https://opentofu.org/][OpenTofu]] is used to manage them in a similar declarative spirit to the
rest of this ecosystem.

#+begin_aside
OpenTofu is a [[https://opentofu.org/manifesto][fork of Terraform]] after the latter's adoption of a BUSL
license.  This fork is new in 2023, and the word ~terraform~ is still
frequently encountered even under OpenTofu.
#+end_aside
...
OpenTofu plugins are called "providers," which are often written by
third parties.  The [[https://registry.terraform.io/providers/timohirt/hetznerdns/latest/docs][~hetznerdns~ provider]] is no exception.

#+begin_src terraform :tangle ../../gen/infra/hetznerdns/terraform.tf
  terraform {
    required_providers {
      hetznerdns = {
...
This provider is better documented in [[https://github.com/timohirt/terraform-provider-hetznerdns/][its README]].
...
The provider must be initialized before first use.
...
#+begin_info
The generated Terraform is in the ~gen/infra/hetznerdns~ directory of
the Cromulent root.  All commands should be run from there.
#+end_info

#+begin_src sh :cache yes :dir ../../gen/infra/hetznerdns :exports both :results verbatim :wrap details :post wrap-example(text=*this*) :code-license CC0-1.0
  tofu init
#+end_src
...
#+begin_aside
The old Java programmer in me is going to give all the OpenTofu
resources a reverse-domain name so they alphabetize neatly, but do
what you like.
#+end_aside
...
The OpenTofu declarations of other records are documented with the
machines or services that need them, but can be found in the same
~gen/infra/hetznerdns~ directory.
...
~plan~ is a dry run.  It will not make any destructive changes.

#+begin_src sh :eval no-export :exports code :dir ../../gen/infra/hetznerdns :results verbatim
  tofu plan
#+end_src

~apply~ applies the plan.  It will reshow the plan and ask for confirmation.

#+begin_warning
Read carefully.  It might delete your records.  I got careless and
lost my first Mastodon instance this way.
#+end_warning

#+begin_src sh :eval no :dir ../../gen/infra/hetznerdns
  tofu apply
#+end_src

*** Gitignores

OpenTofu dumps information about its providers and the known state of
the infrastructure in the same directory as the declarations.  Those
don't belong in git.

#+begin_aside
There's nothing specific to Hetzner DNS here.  These should be wildcard
ignores if we ever add more OpenTofu projects.
#+end_aside

#+begin_src gitignore :tangle ../../.gitignore :code-license CC0-1.0
  gen/infra/hetznerdns/.terraform/
  gen/infra/hetznerdns/*.tfstate
  gen/infra/hetznerdns/*.tfstate.*
#+end_src
...
:header-args:terraform+: :code-license CC0-1.0
:EXPORT_HUGO_LASTMOD: [2023-12-13 Wed 20:54 EST]
:CUSTOM_ID: pobox
:END:

#+begin_description
Defines the DNS entries necessary to forward mail.
#+end_description
...
switched to [[https://www.pobox.com/][pobox.com]] when my forwarder became insufficient.
...
To deliver my mail, these ~MX~ records are required in our [[#dns][Hetzner DNS
project]].
...
All mail from ~rossabaker.com~ goes through ~pobox.com~.  Declaring
this with an [[https://datatracker.ietf.org/doc/html/rfc7208][{{{abbr(Sender Policy Framework,SPF)}}}]] record helps
protect against spoofs and spams.

#+begin_warning
Values containing spaces [[https://github.com/timohirt/terraform-provider-hetznerdns#example-usage][need to be JSON encoded]].
#+end_warning
...
messages with a [[https://datatracker.ietf.org/doc/html/rfc8463][{{{abbr(DomainKeys Identified Mail,DKIM)}}} key]].
The public key needs to be published as a ~TXT~ record:
...
:EXPORT_HUGO_LASTMOD: [2023-12-05 Tue 16:19 EST]
:header-args+: :code-license CC0-1.0
:header-args:nix+: :code-license MIT
:END:
...
:EXPORT_HUGO_LASTMOD: [2023-12-13 Wed 21:09 EST]
:END:
...
*** OpenTofu configuration of =SRV= records
...
=cromulent_avatars_domain= specifies the domain from which the avatars
are to be served.  It must end in a period.  I use
~avatars.rossabaker.com.~.
...
is fine if the former redirecs to the latter.  The three parameters
before the domain name are priority, weight, and port.  Priority and
weight can both be zero.  This configuration is for [[https://www.hetzner.com/dns-console][Hetzner DNS]].
Translate to your own provider as necessary.
...
:EXPORT_HUGO_LASTMOD: [2023-12-13 Wed 16:05]
:END:
