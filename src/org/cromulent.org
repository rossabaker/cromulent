...
:EXPORT_HUGO_LASTMOD: [2023-10-01 Sun 23:32 EDT]
:header-args+: :code-license CC0-1.0
:header-args:nix+: :code-license MIT
:END:
...
Everything Garnix builds goes into the [[https://garnix.io/docs/caching][Garnix cache]].  If you're
consuming my flake, you probably want to configure Nix with the Garnix
substituter and trusted public key.

#+begin_info
These settings are applicable to _all_ Garnix builds, not just my own
flake.
#+end_info
...
#+begin_src nix :tangle ../../gen/flake/garnix/module.nix
  { config, lib, ... }:
...
The module is applicable to NixOS, Nix-Darwin, or Home Manager,
depending on where you configure Nix.  We expose it in the flake as a
standalone =cachix= module and as part of the =default= for each.
...
:EXPORT_HUGO_LASTMOD: [2023-09-24 Sun 22:44 EDT]
:header-args+: :code-license MIT
:EXPORT_OPTIONS: h:6
:END:
