...
:EXPORT_HUGO_LASTMOD: [2023-12-08 Fri 16:40 CST]
:END:
...
This Nix derivation scales a larger picture into the maximum avatar
size.  It assumes the source pictures are squares.
...
- An =md5= and =sha256= hash of the email address, as lower case.

  #+begin_example sh
  echo -n "ross@rossabaker.com" | md5sum
  echo -n "ross@rossabaker.com" | sha256sum
  #+end_example

  The hashes can also be calculated from the [[https://www.libravatar.org/tools/check/][Libravatar check page]].
