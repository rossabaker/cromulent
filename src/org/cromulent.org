...
:EXPORT_HUGO_LASTMOD: [2023-12-07 Thu 22:27 CST]
:CUSTOM_ID: abe
...
This server hosts my [[#libravatar][Libravatar]].
...
:EXPORT_HUGO_LASTMOD: [2023-12-09 Sat 20:02 EST]
:END:
...
This configuration sets up ~avatars.rossabaker.com~ as a domain to
host the avatars.  When the avatar of ~user@example.com~ is requested,
a Libravatar client looks up the [[https://en.wikipedia.org/wiki/SRV_record][{{{abbr(Service,~SRV~)}}} record]] for
~_avatars._tcp.example.com~ (http) or ~avatars-sec._tcp.example.com~
(https).

This OpenTofu configuration defines the necessary records via [[https://www.hetzner.com/dns-console][Hetzner
DNS]].  The first two numbers are the priority and weight, which can both
be zero.  The third number is the port, 80 for http and 443 for https.

#+begin_warning
Don't forget the period at the end of the value!
#+end_warning
...
See the result for yourself with the following command:

#+begin_example sh
  dig _avatars-sec._tcp.rossabaker.com SRV
#+end_example

#+begin_info
  An ~A~ or ~CNAME~ record is still required for
  ~avatars.rossabaker.com~.  I run it on [[#abe][~abe~]].
#+end_info
...
For a single-user Libravatar, we need to specificy two config options:

- =com.rossabaker.services.libravatar.domain= :: the virtual host from
  which ~/avatar/:hash~ will be served.
- =com.rossabaker.services.libravatar.avatars= :: a path that contains
  original images to be prepared and served.
