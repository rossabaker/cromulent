# Copyright 2022-2023, Ross A. Baker <ross@rossabaker.com>
...
:PROPERTIES:
:EXPORT_HUGO_LASTMOD: [2023-08-26 Sat 17:34 EDT]
...
I'm sorry you're reading this instead of building something neat.

- Sharing is cool.  Pay it forward.
- Remixing is fun.  Acknowledge your sources.
- Mundanities are mostly public domain.
- Thinky bits are mostly share-alike.

** The nitty gritty

*** General content

All content on ~rossabaker.com~ is shared under a [[https://creativecommons.org/licenses/by/4.0/][Creative Commons
Attribution-ShareAlike 4.0 International]] (~CC-BY-SA-4.0~) license.

*** Code snippets

Creative Commons licenses are [[https://creativecommons.org/faq/#can-i-apply-a-creative-commons-license-to-software][not ideal for software]].  Code snippets
on this site may be tagged with a [[https://spdx.org/licenses/][SPDX identifier]] indicating a
disjunctive dual license with a secondary option more appropriate for
code.  For example, the following snippet, tagged ~CC0-1.0~, may be
used under either the [[https://spdx.org/licenses/CC0-1.0.html][Creative Commons Zero v1.0 Universal]] license or
the default ~CC-BY-SA-4.0~, at your discretion.
...
The following licenses are applied to portions of this site:

| SPDX ID        | License                                             |
|----------------+-----------------------------------------------------|
| ~CC-BY-SA-4.0~ | [[https://creativecommons.org/licenses/by/4.0/][Creative Commons Attribution-ShareAlike 4.0]] [fn:18] |
...
:EXPORT_HUGO_LASTMOD: [2023-08-26 Sat 18:50 EDT]
:END:
...
:EXPORT_HUGO_LASTMOD: [2023-08-26 Sat 13:54 EDT]
...
Most of my configurations are permissively licensed for their
triviality and your convenience.  This is generally impractical for
Emacs.

- Did you copy from one of the great configurers, like [[https://gitlab.com/protesilaos/dotfiles][Prot]]?  Lots of
  those are GPL.

- How about [[https://github.com/syl20bnr/spacemacs][Spacemacs]] or [[https://github.com/bbatsov/prelude][Prelude]]?  GPL and GPL.

- Did you copy a configuration from the README of one of the myriad
  GPL libraries?  When does fair use become a derivative work?

- What about the venerable [[https://www.emacswiki.org/][Emacs Wiki]]?  Behold all the GPL in that
  footer.

- Did you copy from Stack Overflow?  That's a [[https://stackoverflow.com/help/licensing][share-alike license]],
  compatible with… the GPL.

Didn't do any of those things?  Unless you have a clean room
~init.el~, I'd wager you're unintentionally laundering GPL code.

Use all you like in a private config.  If you're republishing under
another license, be it not on my soul.
...
#+caption: [[https://emacs.stackexchange.com/a/60590][Original]] by vaab on Emacs Stack Exchange, ~CC-BY-SA-4.0~.
#+begin_src emacs-lisp
  (use-package org
    :config
...
header.  This idea comes from [[https://sachachua.com/blog/2023/01/adding-a-custom-header-argument-to-org-mode-source-blocks-and-using-that-argument-during-export/][Sacha Chua]].  First, we need to add it
to =org-babel-exp-code-template=:

#+begin_src emacs-lisp :exports results :results value pp :wrap src emacs-lisp :var template=`,org-babel-exp-code-template
  `(setq org-babel-exp-code-template ,org-babel-exp-code-template)
#+end_src

This part is grody.  We'd like to define a [[https://orgmode.org/worg/dev/org-export-reference.html][derived backend]], but
ox-hugo's export commands are fairly intricate with the subtree
support, and don't offer an obvious way to inject a new backend.
Unfortunately, we resort to mutating the ox-hugo code.  This is fine,
as long as we don't have other ox-hugo sites.

#+caption: [[https://sachachua.com/dotemacs/index.html#org7814eb7][Original]] by Sacha Chua
#+begin_src emacs-lisp :tangle ../../gen/website/code-license-header.el
...
The answer is almost verbatim from Stack Overflow, but put in a
...
** TODO Add gitignore and gitattributes modes so code copying works
