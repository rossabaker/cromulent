...
:EXPORT_HUGO_LASTMOD: <2022-09-16 Fri 08:49>
...
Any file that needs to be checked in, but whose location we're free
to choose, goes to the ~gen~ directory.
#+begin_src gitattributes :tangle ../../.gitattributes
  gen/            -diff -merge linguist-generated=true
#+end_src

Other generated files have hardcoded locations.  Enumerate them here.
#+begin_src gitattributes :tangle ../../.gitattributes
...
These files don't need to be checked in, but are artifacts of things
we do herein.

~nix build~ creates a ~result/~ symlink in the project root.
...
#+begin_src html :tangle ../../.hugo-out/layouts/partials/contact-info-icons.html
  <ul>
...
#+begin_src html :tangle ../../.hugo-out/layouts/shortcodes/contact-info-dl.html
  <dl>
      {{ range .Site.Params.contacts }}
          <dt>{{ .network }}</dt>
