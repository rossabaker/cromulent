...
:EXPORT_DESCRIPTION: An partial enumeration of addresses, social networks, and PGP public key.
:END:
...
:EXPORT_HUGO_LASTMOD: <2022-08-28 Sun 17:41>
...
#+begin_src html :tangle ../../.hugo-out/layouts/partials/article-summary.html
  <article>
          ...
          <ul>
              {{ if (not .Date.IsZero) }}
                  <li class="publish-date">
                      <time title="Original date" datetime="{{ dateFormat "Jan 2, 2006" .Date }}">
                          {{ dateFormat "Jan 2, 2006" .Date }}
                      </time>
                  </li>
              {{ end }}
              {{ if ne (dateFormat "Jan 2, 2006" .Date) (dateFormat "Jan 2, 2006" .Lastmod) }}
                  <li class="last-modified-date">
                      <time title="Last modified date" datetime="{{ dateFormat "Jan 2, 2006" .Lastmod }}">
                          {{ dateFormat "Jan 2, 2006" .Lastmod }}
                      </time>
                  </li>
              {{ end }}
      ...
      <main>
          {{ (.Description | markdownify) | default .Summary }}
      </main>
      <footer>
          <ul>
...
#+begin_src css :tangle ../../.hugo-out/assets/css/cromulent.css
  ...
  ul.article-summaries article {
      border-top: 1px dotted #ccc;
      padding: 1em;
  }

  ul.article-summaries article header h1 {
      font-size: 1.25em;
      margin-bottom: 0.25em;
  }

  ul.article-summaries article main {
      margin: 1em 0;
  }

  ul.article-summaries article header ul, ul.article-summaries article footer ul {
      font-size: 0.8em;
  }
  ...
  ul.article-summaries article header li, ul.article-summaries article footer li {
      display: inline;
      margin-right: 0.5ch;
  }

  ul.article-summaries li.category::before {
      content: "📁 ";
  }

  ul.article-summaries li.tag::before {
      content: "🏷️ ";
  }

  ul.article-summaries li.publish-date::before {
      content: "📅";
  }

  ul.article-summaries li.last-modified-date::before {
      content: "🔄";
  }
#+end_src
